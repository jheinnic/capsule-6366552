...
def drive_encoder( num, funcs, buf ):
    encode_worker = funcs['encode_generator']()
    count = 0
    next(encode_worker)
    for val in num:
        # print(f"Encoding {val} of type {type(val)}")
        byte_out = encode_worker.send(val)
        while not byte_out is None:
            # print(f"Writing {byte_out} of type {type(byte_out)}")
            buf.write(bytes((byte_out,)))
            byte_out = next(encode_worker)
#             count = count + 1
#             if count % 1024 == 0:
#                 print(f"Wrote {count} bytes")
    # encode_worker.send(None)
...
#             counter = counter + 1
#             if counter % 1024:
#                 print(f"Read back {counter} values")
